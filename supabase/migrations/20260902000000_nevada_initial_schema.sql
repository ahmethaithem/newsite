...
create table if not exists public.orders (
  ...
  secondary_phone text check (secondary_phone is null or secondary_phone ~ '^07[0-9]{9}$'),
  ...
  shipping_fee_iqd integer not null default 0 check (shipping_fee_iqd >= 0),
  cod_amount_iqd integer not null check (cod_amount_iqd >= 0),
  total_items integer not null check (total_items > 0),
  status text not null default 'pending' check (
    status in ('pending', 'confirmed', 'ready_for_shipping', 'exported', 'cancelled')
  ),
